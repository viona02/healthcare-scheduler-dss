...
// Set SQLite path
process.env.SQLITE_DATABASE_URL = `file:${path.join(__dirname, '..', 'prisma', 'dev.db')}`;

// Import generated SQLite client
const { PrismaClient } = require('../generated-sqlite-client');
const prisma = new PrismaClient();

async function exportFull() {
  console.log('Reading all data from local dev.db via Raw Queries...');
  ...
  console.log(`Successfully retrieved from dev.db:`);
  console.log(`- ${shifts.length} shifts`);
  console.log(`- ${workers.length} workers`);
  console.log(`- ${users.length} users`);
  console.log(`- ${schedules.length} schedules`);
  console.log(`- ${assignments.length} assignments`);
  console.log(`- ${requests.length} shift requests`);

  // Force schedule with max ID to be isSelected = true so active schedule shows in worker dashboard!
  if (schedules.length > 0) {
    const maxSched = schedules.reduce((max: any, cur: any) => cur.id > max.id ? cur : max, schedules[0]);
    schedules.forEach((s: any) => s.isSelected = false);
    maxSched.isSelected = true;
    console.log(`Marked Schedule ID ${maxSched.id} as isSelected = true`);
  }
...
`;
  ...
  if (shifts.length > 0) {
    ...
    sql += shifts.map((s: any) => `(${s.id}, '${s.name}', '${s.startTime}', '${s.endTime}', ${s.durationHrs}, ${s.minNurses}, ${s.minMidwives}, ${s.minSeniors})`).join(',\n') + `\nON CONFLICT ("id") DO NOTHING;\n\n`;
  }
  ...
  if (workers.length > 0) {
    ...
    sql += workers.map((w: any) => `(${w.id}, '${w.name.replace(/'/g, "''")}', '${w.workerType}', '${w.skillLevel}', ${w.fixedShift ? `'${w.fixedShift}'` : 'NULL'}, ${Boolean(w.weekendHolidayOff)}, ${Boolean(w.sundayHolidayOff)})`).join(',\n') + `\nON CONFLICT ("id") DO NOTHING;\n\n`;
  }

  // Users
  if (users.length > 0) {
    sql += `-- 4. Insert Users\nINSERT INTO "User" ("id", "username", "password", "fullName", "role", "workerId") VALUES\n`;
    sql += users.map((u: any) => `(${u.id}, '${u.username}', '${u.password}', '${u.fullName.replace(/'/g, "''")}', '${u.role}', ${u.workerId ?? 'NULL'})`).join(',\n') + `\nON CONFLICT ("id") DO NOTHING;\n\n`;
  }

  // Schedules
  if (schedules.length > 0) {
    sql += `-- 5. Insert Schedules (${schedules.length} total)\nINSERT INTO "Schedule" ("id", "month", "year", "status", "isSelected", "fitnessScore", "generationCount") VALUES\n`;
    sql += schedules.map((sc: any) => `(${sc.id}, ${sc.month}, ${sc.year}, '${sc.status || 'published'}', ${Boolean(sc.isSelected)}, ${sc.fitnessScore}, ${sc.generationCount})`).join(',\n') + `\nON CONFLICT ("id") DO NOTHING;\n\n`;
  }

  // Assignments (chunked into 100 per statement)
  if (assignments.length > 0) {
    sql += `-- 6. Insert Assignments (${assignments.length} total)\n`;
    const chunkSize = 100;
    for (let i = 0; i < assignments.length; i += chunkSize) {
      ...
      sql += chunk.map((a: any) => `(${a.id}, ${a.scheduleId}, ${a.workerId}, ${a.dayOfMonth}, ${a.shiftId})`).join(',\n') + `\nON CONFLICT ("id") DO NOTHING;\n\n`;
    }
  }

  // ShiftRequests
  if (requests.length > 0) {
    sql += `-- 7. Insert ShiftRequests (${requests.length} total)\nINSERT INTO "ShiftRequest" ("id", "workerId", "date", "endDate", "type", "shiftPref", "reason", "status", "rejectionReason") VALUES\n`;
    sql += requests.map((r: any) => {
...
`;
  ...
  console.log('🎉 SUCCESS! Exported COMPLETE SQL with all schedules & assignments to:', outputPath);
}
