...
function analyzeScheduleDetails(chromosome, workers, shifts, periodDates, holidays, requests, requestLookup) {
    ...
    for (const worker of workers) {
        ...
        for (let day = 0; day < totalDays; day++) {
            ...
            if (sIdx !== -1)
                totalHrs += shifts[sIdx].durationHrs;
        }
        if (totalHrs < 160)
            hardViolations += Math.ceil((160 - totalHrs) / 10);
        if (totalHrs > 180)
            hardViolations += Math.ceil((totalHrs - 180) / 10);
    }
    for (const worker of workers) {
        let consec = 0;
        for (let day = 0; day < totalDays; day++) {
    ...
    for (const req of requests) {
        ...
        const dayIdx = periodDates.findIndex(d => d.getFullYear() === reqDate.getFullYear() &&
...
async function main() {
    ...
    const requestLookup = (0, geneticAlgorithm_1.buildRequestLookup)(requests, periodDates, shifts);
    ...
    const originalWeights = { ...geneticAlgorithm_1.AHP_WEIGHTS };
    for (const sc of scenarios) {
        ...
        geneticAlgorithm_1.AHP_WEIGHTS.equalWorkingHours = sc.weights.equalWorkingHours;
        geneticAlgorithm_1.AHP_WEIGHTS.fulfillingRequests = sc.weights.fulfillingRequests;
        geneticAlgorithm_1.AHP_WEIGHTS.equalNightShifts = sc.weights.equalNightShifts;
        geneticAlgorithm_1.AHP_WEIGHTS.equalWeekendHolidays = sc.weights.equalWeekendHolidays;
        const runs = [];
        for (let r = 1; r <= 1; r++) {
            const startTime = performance.now();
            const result = (0, geneticAlgorithm_1.runGeneticAlgorithm)(workers, shifts, periodDates, requests, holidays, GA_TEST_CONFIG);
            const endTime = performance.now();
            const compTimeMs = endTime - startTime;
            const details = analyzeScheduleDetails(result.bestSchedule, workers, shifts, periodDates, holidays, requests, requestLookup);
            const runData = {
                run: r,
...
//# sourceMappingURL=benchmarkAhpRunner.js.map