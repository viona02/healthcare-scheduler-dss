...
exports.DEFAULT_GA_CONFIG = exports.AHP_WEIGHTS = void 0;
exports.buildPeriodDates = buildPeriodDates;
exports.buildRequestLookup = buildRequestLookup;
exports.cloneChromosome = cloneChromosome;
exports.cloneDay = cloneDay;
...
function buildPeriodDates(month, year) {
...
function isWorkerOffOnDay(w, date, holidays) {
    if (w.sundayHolidayOff && isSundayOrHoliday(date, holidays))
        return true;
    if (w.weekendHolidayOff && isHolidayOrWeekend(date, holidays))
        return true;
    return false;
}
// Dapatkan shift index sebuah worker di hari tertentu, atau -1 jika libur
function getWorkerShiftIndex(chromosome, day, workerId) {
...
function isWorkerWorking(chromosome, day, workerId) {
...
function buildRequestLookup(requests, periodDates, shifts) {
...
function createRandomChromosome(workers, shifts, totalDays, periodDates, holidays, requestLookup) {
    const chromosome = [];
    ...
    const isEligible = (w, day, shiftName) => {
        ...
        if (w.fixedShift && w.fixedShift !== shiftName)
            return false;
        return true;
    };
    for (let day = 0; day < totalDays; day++) {
        const dayGene = [];
        const dayReqs = requestLookup.get(day);
        const offWorkerIds = dayReqs?.offWorkerIds || new Set();
        ...
        for (let s = 0; s < shifts.length; s++) {
            ...
            for (const wId of prefWorkerIds) {
                ...
                if (!w || !isEligible(w, day, shift.name))
                    continue;
            ...
            const availableNurses = shuffleArray(nurses.filter(n => !assignedToday.has(n.id) && isEligible(n, day, shift.name)));
            let nursesAssigned = assignedWorkers.filter(id => nurses.some(n => n.id === id)).length;
            for (const nurse of availableNurses) {
                if (nursesAssigned >= shift.minNurses)
                    break;
            ...
            const targetMidwives = (shift.name === 'Pagi' && isSundayOrHoliday(periodDates[day], holidays))
                ? 0
                : shift.minMidwives;
            const availableMidwives = shuffleArray(midwives.filter(m => !assignedToday.has(m.id) && isEligible(m, day, shift.name)));
            let midwivesAssigned = assignedWorkers.filter(id => midwives.some(m => m.id === id)).length;
            for (const midwife of availableMidwives) {
                if (midwivesAssigned >= targetMidwives)
                    break;
...
function initializePopulation(workers, shifts, totalDays, periodDates, holidays, populationSize, requestLookup) {
    const population = [];
...
function checkHardConstraints(chromosome, workers, shifts, periodDates, holidays, requestLookup) {
    let violations = 0;
    let penalty = 0;
    ...
    // HC2: No double shift
    for (let day = 0; day < totalDays; day++) {
        const workerShiftCount = new Map();
        for (let s = 0; s < shifts.length; s++) {
    ...
    if (nightShiftIndex !== -1) {
        for (const worker of workers) {
            ...
            while (i < totalDays) {
                if (nightArr[i]) {
                    let runLen = 0;
                    while (i + runLen < totalDays && nightArr[i + runLen])
                        runLen++;
                    if (runLen !== 2) {
    ...
    for (const worker of workers) {
        ...
        for (let day = 0; day < totalDays; day++) {
            ...
            if (sIdx !== -1)
                totalHours += shifts[sIdx].durationHrs;
        }
        if (totalHours < 160) {
...
function calculateSoftConstraints(chromosome, workers, shifts, periodDates, holidays, requests) {
    const totalDays = chromosome.length;
    let fitness = 0;
    ...
    fitness += exports.AHP_WEIGHTS.equalWorkingHours * hoursScore;
    // ===== A2: Fulfilling Health Workers Requests (bobot: 0.11) =====
    let totalRequests = requests.length;
    let fulfilledRequests = 0;
    for (const req of requests) {
        const reqDate = new Date(req.date);
        const dayIndex = periodDates.findIndex(d => d.getFullYear() === reqDate.getFullYear() &&
    ...
    fitness += exports.AHP_WEIGHTS.fulfillingRequests * requestScore;
    // ===== A3: Equal Distribution of Night Shifts (bobot: 0.43) =====
    const nightShiftIndex = shifts.findIndex(s => s.name === 'Malam');
    if (nightShiftIndex !== -1) {
        const nightCountPerWorker = new Map();
        for (const worker of workers) {
        ...
        fitness += exports.AHP_WEIGHTS.equalNightShifts * nightScore;
    }
    // ===== A4: Equal Distribution of Weekend Holidays (bobot: 0.04) =====
    const weekendOffPerWorker = new Map();
    for (const worker of workers) {
    ...
    fitness += exports.AHP_WEIGHTS.equalWeekendHolidays * weekendScore;
    return fitness;
}
...
function mutate(chromosome, rate, workers, shifts, periodDates, holidays, requestLookup) {
    const mutated = cloneChromosome(chromosome);
    const totalDays = mutated.length;
    const workerById = new Map(workers.map(w => [w.id, w]));
    const isWorkerLocked = (w, day) => {
        if (isWorkerOffOnDay(w, periodDates[day], holidays))
            return true;
        if (w.fixedShift)
            return true; // Fixed shift (Rika & Livia)
        const dayReqs = requestLookup?.get(day);
        if (dayReqs?.offWorkerIds.has(w.id))
            return true;
    ...
    const canPlace = (w, day, shiftName) => {
        if (isWorkerOffOnDay(w, periodDates[day], holidays))
            return false;
        if (w.fixedShift && w.fixedShift !== shiftName)
            return false;
        ...
        if (Math.random() < rate * 0.5) {
            ...
            if (mutated[day][shiftIdx].length > 0) {
                ...
                if (oldWorker && !isWorkerLocked(oldWorker, day)) {
                    const freeSameType = workers.filter(w => !assignedToday.has(w.id) &&
...
function repairChromosome(chromosome, workers, shifts, periodDates, holidays, requestLookup) {
    const totalDays = chromosome.length;
    const nurses = workers.filter(w => w.workerType === 'perawat');
    const midwives = workers.filter(w => w.workerType === 'bidan');
    const nightShiftIndex = shifts.findIndex(s => s.name === 'Malam');
    const morningShiftIndex = shifts.findIndex(s => s.name === 'Pagi');
    const isWorkerLockedOnDay = (w, day) => {
        if (isWorkerOffOnDay(w, periodDates[day], holidays))
            return true;
        if (w.fixedShift)
            return true;
    ...
    for (let day = 0; day < totalDays; day++) {
        ...
        for (let s = 0; s < shifts.length; s++) {
            chromosome[day][s] = chromosome[day][s].filter(wId => {
                ...
                if (isWorkerOffOnDay(w, periodDates[day], holidays))
                    return false;
                if (dayReqs?.offWorkerIds.has(wId))
                    return false;
                return true;
            });
        }
    ...
    if (nightShiftIndex !== -1) {
        for (const worker of workers) {
            if (worker.fixedShift)
                continue; // Rika & Livia selalu Pagi
            let day = 0;
            while (day < totalDays) {
    ...
    for (let day = 0; day < totalDays; day++) {
        ...
        for (let s = 0; s < shifts.length; s++) {
            ...
            const isCandidateEligible = (w) => {
                if (assignedToday.has(w.id))
                    return false;
                if (isWorkerLockedOnDay(w, day))
                    return false;
                if (!canPlace(w, day, shift.name))
                    return false;
                // Aturan Malam -> Pagi
                ...
                if (nightShiftIndex !== -1) {
                    if (day >= 2 && chromosome[day - 1][nightShiftIndex].includes(w.id) && chromosome[day - 2][nightShiftIndex].includes(w.id))
                        return false;
                    if (day >= 3 && chromosome[day - 2][nightShiftIndex].includes(w.id) && chromosome[day - 3][nightShiftIndex].includes(w.id))
                        return false;
                }
                // Aturan 6 Hari Berturut-turut
                let cons = 0;
                for (let d = day - 1; d >= Math.max(0, day - 6); d--) {
                    if (isWorkerWorking(chromosome, d, w.id))
                        cons++;
                    else
                        break;
                }
...
function calculateFitness(individual, workers, shifts, periodDates, holidays, requests, requestLookup) {
...
// ----- Main GA Loop -----
function runGeneticAlgorithm(workers, shifts, periodDates, requests, holidays, config = exports.DEFAULT_GA_CONFIG, onProgress) {
    ...
    // 3. Evolusi
    let bestEver = { chromosome: [], fitness: -Infinity };
    ...
    for (let gen = 0; gen < config.maxGenerations; gen++) {
        ...
        while (newPopulation.length < config.populationSize) {
            ...
            const ind1 = { chromosome: mutChild1, fitness: 0 };
            const ind2 = { chromosome: mutChild2, fitness: 0 };
