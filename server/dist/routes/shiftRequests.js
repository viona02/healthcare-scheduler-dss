...
router.get('/', async (req, res) => {
    try {
        ...
        const requests = await prisma_1.default.shiftRequest.findMany({
            where,
            include: { worker: true },
...
router.post('/', async (req, res) => {
    try {
        ...
        const existingInPeriod = await prisma_1.default.shiftRequest.findMany({
            where: {
                workerId,
        ...
        const request = await prisma_1.default.shiftRequest.create({
            data: {
                workerId,
                date: startDate,
...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        if (status === 'approved') {
            const targetRequest = await prisma_1.default.shiftRequest.findUnique({
            ...
            if (targetRequest.type === 'off') {
                ...
                while (checkCurrent <= reqEnd) {
                    const sameDayApproved = await prisma_1.default.shiftRequest.findMany({
                        where: {
                            status: 'approved',
                            type: 'off',
        ...
        const request = await prisma_1.default.shiftRequest.update({
            where: { id: parseInt(req.params.id) },
            data: {
                status,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await prisma_1.default.shiftRequest.delete({ where: { id } });
...
//# sourceMappingURL=shiftRequests.js.map