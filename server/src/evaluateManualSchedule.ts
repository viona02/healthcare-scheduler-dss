...
async function main() {
  ...
  const periodDates = buildPeriodDates(6, 2026);
  ...
  const requests: ShiftRequestData[] = [
  ...
  for (let day = 0; day < totalDays; day++) {
    const dayGene: number[][] = [[], [], []]; // Pagi=0, Siang=1, Malam=2
    for (let wIdx = 0; wIdx < workers.length; wIdx++) {
      const workerId = workers[wIdx].id;
      const code = rawSchedule[wIdx][day];
      if (code === 'P') dayGene[0].push(workerId);
      else if (code === 'S') dayGene[1].push(workerId);
      else if (code === 'M') dayGene[2].push(workerId);
    }
  ...
  for (const req of requests) {
    ...
    for (let dIdx = 0; dIdx < totalDays; dIdx++) {
      ...
      if (dStr >= startStr && dStr <= endStr) {
        ...
        if (req.type === 'off') {
          if (sIdx === -1) fulfilledReqEntries++;
        } else if (req.type === 'preference' && req.shiftPref) {
          if (sIdx !== -1 && shifts[sIdx].name === req.shiftPref) fulfilledReqEntries++;
        }
      }
    }
  }
  ...
  const totalFitnessScore =
    AHP_WEIGHTS.equalWorkingHours * scoreA1 +
    AHP_WEIGHTS.fulfillingRequests * scoreA2 +
    AHP_WEIGHTS.equalNightShifts * scoreA3 +
    AHP_WEIGHTS.equalWeekendHolidays * scoreA4;
  ...
  for (let day = 0; day < totalDays; day++) {
    ...
    for (let s = 0; s < 3; s++) {
      for (const id of chromosome[day][s]) counts.set(id, (counts.get(id) || 0) + 1);
    }
  ...
  const reqLookup = buildRequestLookup(requests, periodDates, shifts);
  const ind = { chromosome, fitness: 0 };
  const rawTotalFitness = calculateFitness(ind, workers, shifts, periodDates, holidays, requests, reqLookup);
